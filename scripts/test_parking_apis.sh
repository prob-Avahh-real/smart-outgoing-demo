...
RESPONSE=$(curl -s -X POST "$BASE_URL/api/parking/find" \
...
RESPONSE=$(curl -s "$BASE_URL/api/parking/lots")

if echo "$RESPONSE" | jq -e '.parking_lots' > /dev/null 2>&1; then
...
RESPONSE=$(curl -s "$BASE_URL/api/parking/lots/lot_1")

if echo "$RESPONSE" | jq -e '.parking_lot' > /dev/null 2>&1; then
...
RESPONSE=$(curl -s "$BASE_URL/api/parking/lots/lot_1/spaces")

if echo "$RESPONSE" | jq -e '.parking_spaces' > /dev/null 2>&1; then
...
RESPONSE=$(curl -s -X POST "$BASE_URL/api/parking/reserve" \
  -H "Content-Type: application/json" \
  -H "x-user-id: $USER_ID" \
  -d '{
...
if echo "$RESPONSE" | jq -e '.reservation' > /dev/null 2>&1; then
...
RESPONSE=$(curl -s -X POST "$BASE_URL/api/parking/session/start" \
  -H "Content-Type: application/json" \
  -H "x-user-id: $USER_ID" \
  -d '{
    "parking_lot_id": "lot_1",
...
if echo "$RESPONSE" | jq -e '.session' > /dev/null 2>&1; then
...
if echo "$RESPONSE" | jq -e '.amap_js_key' > /dev/null 2>&1; then
